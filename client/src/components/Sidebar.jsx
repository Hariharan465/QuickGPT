    ...
    border-r border-[#80609f]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      <div className="flex items-center gap-2 p-3 mt-4 border border-gray-400 dark:border-white/20 rounded-md">
        ...
        <input
          ...
          value={search}
          type="text"
          placeholder="Search Connversations"
...
<div className="flex-1 overflow-y-scroll mt-3 text-sm space-y-3">
  {
    chats.filter((chat) => {
      // Safely check if messages exist and have at least one item
      const firstMessage = chat?.messages?.[0];
      const chatName = chat?.name || '';
      const searchText = search?.toLowerCase() || '';
      
      if (firstMessage?.content) {
        return firstMessage.content.toLowerCase().includes(searchText);
      } else {
        return chatName.toLowerCase().includes(searchText);
      }
    }).map((chat) => (
      <div 
        onClick={() => {
 ...
 {/* Credit Purchase Option */}
  ...
  rounded-md cursor-pointer hover:scale-103 transition-all">
    ...
    <div className="flex flex-col text-sm">
      ...
      <p className="text-sm text-gray-400">Purchase credits to use quickGPT</p>
    </div>
  </div>
